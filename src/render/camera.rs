...
impl Camera {
    ...
    pub fn set_view(&mut self, position: Vec3, target: Vec3, up: Vec3) {
        ...
        self.view = Mat4::look_at(
            Pt3::from_vec(self.position),
            Pt3::from_vec(self.target),
            self.up,
        );
        self.update_screen2ray();
        self.update_frustrum();
    ...
    pub fn view_direction_at(&self, screen_coordinates: (f64, f64)) -> Vec3 {
        let screen_pos = vec4(
            2. * screen_coordinates.0 as f64 - 1.,
            1. - 2. * screen_coordinates.1 as f64,
            0.,
            1.,
        );
    ...
    pub fn in_frustrum(&self, min: &Vec3, max: &Vec3) -> bool {
        ...
        for i in 0..6 {
            let mut out = 0;
            if self.frustrum[i].dot(vec4(min.x, min.y, min.z, 1.0)) < 0.0 {
                out += 1
            };
            if self.frustrum[i].dot(vec4(max.x, min.y, min.z, 1.0)) < 0.0 {
                out += 1
            };
            if self.frustrum[i].dot(vec4(min.x, max.y, min.z, 1.0)) < 0.0 {
                out += 1
            };
            if self.frustrum[i].dot(vec4(max.x, max.y, min.z, 1.0)) < 0.0 {
                out += 1
            };
            if self.frustrum[i].dot(vec4(min.x, min.y, max.z, 1.0)) < 0.0 {
                out += 1
            };
            if self.frustrum[i].dot(vec4(max.x, min.y, max.z, 1.0)) < 0.0 {
                out += 1
            };
            if self.frustrum[i].dot(vec4(min.x, max.y, max.z, 1.0)) < 0.0 {
                out += 1
            };
            if self.frustrum[i].dot(vec4(max.x, max.y, max.z, 1.0)) < 0.0 {
                out += 1
            };
            if out == 8 {
                return false;
