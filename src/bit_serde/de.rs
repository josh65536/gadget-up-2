use bitvec::prelude::*;
use serde::de::{self, DeserializeSeed, EnumAccess, MapAccess};
...
impl<'de> Deserializer<'de> {
    pub fn from_bits(input: &'de BitSlice) -> Self {
        Deserializer { input }
    }
}

pub fn from_bits<'a, T>(bits: &'a BitSlice) -> Result<T>
where
    T: Deserialize<'a>,
{
...
impl<'de> Deserializer<'de> {
    ...
    fn parse_uint(&mut self) -> Result<u64> {
        ...
        if self.input.len() < 2 * ones + 1 {
            // too small to fit number
        ...
        let res = if ones == 0 {
            ...
            let lo = self.input[(ones + 1)..((2 * ones + 1).min(ones + 1 + 32))].load_le::<u32>();
            let hi = if ones + 1 + 32 >= 2 * ones + 1 {
                0
            } else {
                self.input[(ones + 1 + 32)..(2 * ones + 1)].load_le::<u32>()
            };
...
impl<'de, 'a> de::Deserializer<'de> for &'a mut Deserializer<'de> {
    ...
    where
        V: Visitor<'de>,
    {
        // This had better be an enum variant and not a struct field
...
mod test {
    ...
    fn test_uint() {
        ...
        assert_eq!(
            from_bits::<u64>(bits![1, 1, 1, 1, 0, 0, 1, 0, 0]).unwrap(),
            17u64
        );
    }
    ...
    fn test_option() {
        ...
        assert_eq!(
            from_bits::<Option<u64>>(bits![1, 1, 0, 0]).unwrap(),
            Some(1)
        );
    }
    ...
    fn test_seq() {
        ...
        assert_eq!(
            from_bits::<Vec<u64>>(bits![1, 1, 0, 0, 0, 1, 1, 0, 1, 1, 1, 0, 1, 0]).unwrap(),
            vec![6u64, 2, 0]
        );
    }
    ...
    fn test_tuple() {
        ...
        assert_eq!(
            from_bits::<(u64, u64, i64)>(bits![1, 1, 0, 1, 1, 1, 0, 1, 0, 0]).unwrap(),
            (6u64, 2u64, 0i64)
        );
    }
}
